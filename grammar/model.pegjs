...
validate
 = space? "validate" eol? "{" eol? v:validations eol? space* "}" eol? { return v; }


validations
  = space* val:(
    first: (a:Validate eol? { return a;} )
    rest: (space* eol? ","? eol? a:Validate { return a; })* { return [first].concat(rest);}
  ) { return val; }
...
Type
 = "[" space? t:TypeToken space? "]" { return { repeated: true, name:t }; }
 / t:TypeToken { return { repeated: false, name: t}; }
